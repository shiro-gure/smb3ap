...
# --- Fortress path-locks (level_access): panels that sit behind a fortress on the map ---
# Generated by patch/gen_locks.py from the disasm map grids. Only worlds where a fortress
# genuinely dead-ends the path (no alternate route) appear — e.g. World 6's 6-5+ behind
# 6-F1. Each entry: gated panel base location -> list of fortress clear-check locations
# that must be cleared to physically reach it.
from .locks import PANEL_REQUIRES_FORTRESS  # noqa: E402


def fortress_beaten_event_name(fortress_location: str) -> str:
    """Event item/location name for 'this fortress has been beaten' (logic access)."""
    return fortress_location.replace(" - Cleared", "") + " Beaten (event)"


def required_fortress_events(panel_base_location: str) -> "List[str]":
    """Fortress-beaten EVENT names a gated panel requires to be physically reachable
    (empty for panels with no lock in front of them). Only meaningful with level_access."""
    return [fortress_beaten_event_name(f)
            for f in PANEL_REQUIRES_FORTRESS.get(panel_base_location, [])]


def all_required_fortress_locations() -> "List[str]":
    """Every fortress clear-check location that gates at least one panel (dedup, stable)."""
    seen = []
    for reqs in PANEL_REQUIRES_FORTRESS.values():
        for f in reqs:
            if f not in seen:
                seen.append(f)
    return seen
